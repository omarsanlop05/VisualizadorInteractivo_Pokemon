    ...
    public class ProductPlacement : MonoBehaviour
    {
            ...
            if (IsPlaced)
            {
                if (IsSingleFingerDragging || (VuforiaRuntimeUtilities.IsPlayMode() && Input.GetMouseButton(0)))
                {
                    ...
                    if (Physics.Raycast(_ray, out _hit))
                    {
                        ...
                        {
                            product.gameObject.transform.position = _hit.point;
                        }
                    }
                }
            }
        ...
        private void HandleAutomaticHitTest(HitTestResult result)
        {
            if (!IsPlaced)
            {
                ...
                product.gameObject.transform.position = result.Position;
            }
        }
        ...
        private void RotateTowardCamera()
        {
            if (Camera.main != null)
            {
                _lookAtPosition = Camera.main.transform.position - product.position;
                _lookAtPosition.y = 0;
